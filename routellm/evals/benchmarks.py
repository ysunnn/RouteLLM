...
class JUDGEDGSM8K(Benchmark):
    def __init__(self, routed_pair, overwrite_cache):
        self.routed_pair = routed_pair
        self.overwrite_cache = overwrite_cache
        self.cache_path = f"{CURRENT_DIR}/gsm8k_rerouted/cache.npy"
    ...
    def evaluate(self, controller, router, num_results, overwrite_router_cache):
        ...
        for i, threshold in enumerate(thresholds):
            ...
            final_selection = selection.copy()

            if "Mixtral-8x7B-Instruct-v0.1_response_judge_score" in self.all_data.columns:
                weak_selected = ~selection
                weak_scores = self.all_data.loc[weak_selected, "Mixtral-8x7B-Instruct-v0.1_response_judge_score"]

                # Override if judge score < 0.85
                reroute_to_strong = weak_scores < 0.85
                final_selection.loc[weak_selected] = reroute_to_strong
            ...
            models = np.where(final_selection, self.routed_pair.strong, self.routed_pair.weak)
            model_counts = Counter(models)
            yield threshold, sum(results) / len(results) * 100, model_counts, len(
                results
            )
